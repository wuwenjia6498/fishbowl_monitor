...
# -*- coding: utf-8 -*-
"""
鱼盆趋势雷达 - 数据库初始化脚本 v5.3 (全球指数与贵金属现货扩展)
功能：
1. 宽基大势：监控原生指数 + 全球指数 + 贵金属现货
2. 行业轮动：监控交易型 ETF
3. 多接口路由：index_daily(A股) + index_global(全球) + sge_daily(贵金属)
4. 每个资产有固定的 sort_rank，保证顺序稳定
5. 每个 ETF 包含定制化的投资逻辑说明（Markdown格式）
"""
...
BROAD_INDICES = [
    ...
    {"sort_id": 30, "code": "Au99.99", "name": "黄金现价", "group": "贵金属现货", "etf_label": "黄金"},
    {"sort_id": 31, "code": "Ag(T+D)", "name": "白银现价", "group": "贵金属现货", "etf_label": "白银"},
]
...
目前市场上规模最大、流动性最好的游戏主题ETF，规模一度突破90亿元，且资金常呈现持续净流入态势，是资金配置游戏传媒板块的首选工具。

**资金风向标**

该ETF具有极高的筹码博弈特征。作为TMT板块中高弹性的代表，它常被资金视作捕捉**"底部反转"行情的利器**。每当板块估值回落至相对低位时，往往能吸引"聪明钱"持续逆势布局，呈现出显著的**越跌越买、底部吸金**的资金共识，是市场情绪的核心温度计。

**持仓逻辑**

持仓高度集中于三七互娱、恺英网络、世纪华通等行业龙头。这些企业不仅受益于版号发放常态化带来的供给侧改善，重塑了行业健康发展预期，更承载了市场对"AI+游戏"新玩法的估值重塑预期。游戏被视为AI技术应用落地最快、最具备商业化潜力的场景之一，AI技术能够极大提升游戏的研发效率和用户体验，为行业带来第二增长曲线。"""
    },
    {
        "sort_id": 106, "code": "515880.SH", "name": "通信设备", "group": "科技 (TMT)", "etf_label": "通信ETF",
        "investment_logic": """**规模与地位**

该基金是目前市场上规模最大、流动性最佳的通信主题ETF之一，素有"通信板块风向标"之称。其日均成交额活跃，能够承载较大体量的资金进出，是机构和个人投资者配置通信产业链的首选工具。

**持仓逻辑**

该ETF跟踪中证全指通信设备指数，其核心逻辑已从传统的5G基站建设转向**"AI算力+数字经济"**双轮驱动。

AI算力底座（CPO概念）：重仓了全球光模块龙头（如中际旭创、新易盛），光模块作为AI数据中心内数据传输的"大动脉"，其需求随AI大模型训练呈爆发式增长，是该ETF最大的进攻性来源。

ICT设备与基础设施：覆盖了通信设备巨头（如中兴通讯），提供了业绩的安全垫，同时受益于6G预研、卫星互联网及运营商资本开支的稳健增长。

**市场情绪指标**

该ETF具有极高的高弹性特征。在AI算力行情爆发时，由于光模块板块的业绩兑现能力最强，该ETF往往能跑出超越市场的超额收益（Alpha）。它是观察AI硬件端景气度的核心窗口，当前其偏离度较高（9.33%），显示出极强的上升动能和资金关注度。"""
    },
...
def main():
    print("=" * 60)
    print("鱼盆趋势雷达 - 数据库初始化 v5.3")
    print("全球指数与贵金属现货扩展 + 多接口路由")
    print("=" * 60)
    ...
    try:
        ...
        print("=" * 60)
        print("\n💡 提示：")
        print("  - A股指数使用 pro.index_daily 接口")
        print("  - 全球指数使用 pro.index_global 接口")
        print("  - 贵金属现货使用 pro.sge_daily 接口")
